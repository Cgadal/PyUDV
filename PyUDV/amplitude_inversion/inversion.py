...
def explicit_inversion(MSV, r, Xi, alpha_w, psi, C0, r0, delta_r=None):
    r_expanded = np.expand_dims(np.squeeze(r), axis=tuple(np.arange(len(MSV.shape[:-1]))))
    psi_expanded = np.expand_dims(np.squeeze(psi), axis=tuple(np.arange(len(MSV.shape[:-1]))))
    ...
    r0_brod = np.broadcast_to(r0, f.shape) if np.isscalar(r0) else np.broadcast_to(r0.squeeze()[..., None], f.shape)
    r_temp = np.broadcast_to(r_expanded, f.shape)
    if delta_r is None:  # if single fixed point
        f_zeroed = np.where(r_temp <= r0_brod, f, 0)  # putting 0 everywhere r < r0 to compute constant integrals
        ...
        f_zeroed = np.where(r_temp <= r0_brod + delta_r, f, 0)  # putting 0 everywhere r < r0 + delta_r to compute constant integrals
        constant_int_pos = trapezoid(f_zeroed, r_expanded)[..., None]
        f_zeroed = np.where(r_temp <= r0_brod - delta_r, f, 0)  # putting 0 everywhere r < r0 - delta_r to compute constant integrals
        constant_int_neg = trapezoid(f_zeroed, r_expanded)[..., None]
        #
        B = 4*Xi*(constant_int_pos - np.exp(-8*delta_r*Xi*C0_expanded)*constant_int_neg)/(1 - np.exp(-8*delta_r*Xi*C0_expanded))
